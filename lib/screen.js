...
function displayHelperPath() {
  return path.join(__dirname, 'display-helper');
}

function compileDisplayHelper() {
  const helperPath = displayHelperPath();
  const srcPath = path.join(__dirname, 'display-helper.swift');
  const result = runShell(`swiftc -O "${srcPath}" -o "${helperPath}"`, { timeout: 60000 });
...
// Get display info
function displays() {
  let helperPath = displayHelperPath();
  const fs = require('fs');
  if (!fs.existsSync(helperPath)) {
    const compiled = compileDisplayHelper();
    if (typeof compiled === 'object' && compiled.error) return compiled;
    helperPath = compiled;
  }

  const result = run(helperPath, [], { timeout: 15000 });
  if (typeof result === 'object' && result.error) return result;

  try {
    return JSON.parse(result).map((display, index) => ({
      id: index + 1,
      ...display,
    }));
  } catch {
    return { error: 'Display helper returned malformed JSON' };
  }
}
